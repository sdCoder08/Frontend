...
const MainEvents = ({ events, setEvents, handleVisibleForm }) => {
    ...
    useEffect(() => {
        const getEvent = async () => {
            try {
                const response = await axios.get('http://localhost:4002/Event/GetEvent', { params: { userId } })
    ...
    const deleteEvent = async (index) => {
        ...
        try {
            const response = await axios.delete(`http://localhost:4002/Event/DeleteEvent`, {
