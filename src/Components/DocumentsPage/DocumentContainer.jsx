...
const DocumentContainer = ({ documents, setDocuments }) => {
  ...
  useEffect(() => {
    axios.get('http://localhost:4002/Documents/getDocuments', { params: { userId: userId } })
      .then((response) => {
  ...
  const onDelete = async (title) => {
    try {
      const response = await axios.delete('http://localhost:4002/Documents/deleteDocument', {params: {userId:userId , documentToDelete : title}} )
