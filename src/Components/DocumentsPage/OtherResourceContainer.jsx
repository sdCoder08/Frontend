...
const OtherResourceContainer = ({ webSites, setWebsites, Books, setBooks, Videos, setVideos, handleVisibleForm }) => {
    ...
    useEffect(() => {
        const getBooks = async () => {
            try {
                const response = await axios.get('http://localhost:4002/Books/GetBooks', { params: { userId } })
    ...
    useEffect(() => {
        const getVideos = async () => {
            try {
                const response = await axios.get('http://localhost:4002/Videos/GetVideos', { params: { userId } })
    ...
    useEffect(() => {
        const getWebsites = async () => {
            try {
                const response = await axios.get('http://localhost:4002/Websites/GetWebsites', { params: { userId } })
    ...
    const deleteBook = async (index) => {
        ...
        try {
            const response = await axios.delete('http://localhost:4002/Books/DeleteBook', { params: { userId:  userId, bookTitle: bookToDelete } })
    ...
    const deleteVideo = async (index) => {
        ...
        try {
            const response = await axios.delete('http://localhost:4002/Videos/DeleteVideo', { params: { userId: userId, videoTitle: videoToDelete } })
            setVideos(response.data)
        } catch (error) {
            console.log(error)
        }
    ...
    const deleteWebsite = async (index) => {
        ...
        try {
            const response = await axios.delete('http://localhost:4002/Websites/DeleteWebsite', { params: { userId: userId, websiteTitle: websiteToDelete } })
            setWebsites(response.data)
        } catch (error) {
            console.log(error)
        }
